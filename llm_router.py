...
Fallback chain: role-key → KEY_1 → OPENAI_API_KEY (never crashes).
"""
...
_DEFAULT_MODEL = os.getenv("LLM_MODEL", "MiniMax-M2.5")
...
def get_client(agent_role: str) -> Tuple[OpenAI, str]:
    ...
    if not api_key:
        ...
        if api_key:
            ...
            logger.error("[LLMRouter] No API key found for role '%s'", agent_role)

    model = os.getenv(model_env, _DEFAULT_MODEL)
...
def get_model(agent_role: str) -> str:
    model_env = ROLE_TO_MODEL_ENV.get(agent_role, "LLM_MODEL")
    return os.getenv(model_env, _DEFAULT_MODEL)
